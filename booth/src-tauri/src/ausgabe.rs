...
pub fn lege_ab(ordner: &Path, kennung: &str, daten: &[u8]) -> std::io::Result<PathBuf> {
    fs::create_dir_all(ordner)?;
    let ziel = ordner.join(format!("{}.jpg", saubere_kennung(kennung)));
    fs::write(&ziel, daten)?;
    Ok(ziel)
}
...
pub fn starte_dienst(ordner: PathBuf, port: u16) -> std::io::Result<u16> {
    ...
    thread::spawn(move || {
        for anfrage in server.incoming_requests() {
            ...
            let _ = match antwort {
                Antwort::Datei(daten) => anfrage.respond(
                    tiny_http::Response::new(
                        tiny_http::StatusCode(200),
                        vec![
                            kopfzeile("Content-Type", "image/jpeg"),
...
fn beantworte(ordner: &Path, pfad: &str) -> Antwort {
    ...
    let kennung = saubere_kennung(rest.trim_end_matches(".jpg"));
    if kennung.is_empty() {
        return Antwort::Fehlt;
    }

    match fs::read(ordner.join(format!("{}.jpg", kennung))) {
        Ok(daten) => Antwort::Datei(daten),
        Err(_) => Antwort::Fehlt,
    }
}
...
mod tests {
    ...
    fn legt_ab_und_liefert_aus() {
        ...
        lege_ab(&ordner, "test-1", b"BILDDATEN").expect("ablegen");
