...
darin, dass das Studio das geöffnete Dokument liest und von sich aus sagt,
was zu tun ist.
...
Unterschriftsfeld. Damit lässt sich jede Fähigkeit des Studios ausprobieren,
...
- **Die Schriften liegen bei, nicht bei Google.** Das Handoff nennt Google
  Fonts; das Studio darf nichts nachladen. Acht Schnitte als woff2 in
...
PDF Studio mehr kann als das gezeichnete Produkt, wird das Muster erweitert,
nicht gebrochen.
...
Ein Team-Merkmal, das ohne Server nicht geht, wäre in dieses Studios eine
Attrappe.
...
Projektanweisung ausschließt — also arbeitet dieselbe PDF Studio auf einem
...
Nach dem Laden liest das Studio bis zu 60 Seiten und zieht daraus Schlüsse.
...
Dialog, bevor jemand auf „Erstellen" drückt. Eine Word-Datei auf das Studio
...
**Kennwort und Rechte** — geschützte Dateien öffnen (das Studio fragt nach
...
ohne Text, Bilder. Was ohne Vermutung zu setzen ist, setzt das Studio auf
...
- **Nur eine Unterschrift je Datei.** Eine zweite über die erste hinweg
  ...
  geschrieben werden. Das Studio kann das nicht — und sagt es, statt die
...
- **Kein Zeitstempel von einem Dienst und keine Sperrlistenabfrage.** Die
  ...
  nichts abhängen.
- **„Qualifiziert" entscheidet das Zertifikat.** Das Studio rechnet die
...
```
index.html          Gerüst
manifest.json       macht das Studio installierbar (Name, Symbole, Verknüpfungen)
dienst.js           Service Worker: legt das Studio aufs Gerät, damit es ohne Netz läuft
...
app/dialoge.js      alles, was sich als Fenster ueber das Studio legt
...
Das Studio wählt selbst und sagt im Sicherungsdialog, welcher Weg gilt:
...
```html
<meta name="studio-fremd" content="https://beispiel.example/studio-fremd/">
```
...
## Aufdruck, Felderkennung, Absätze

Drei Fähigkeiten, die sich einen Gedanken teilen: sie arbeiten am ganzen
Dokument, nicht an einer Stelle darauf.

### Aufdruck — Wasserzeichen, Kopf- und Fußzeile, Seitenzählung

`Seiten → Wasserzeichen, Kopf- und Fußzeile`. Eine Beschreibung, keine Liste
von Objekten: sie gilt für jede Seite im Bereich. Gerechnet wird sie zweimal
aus derselben Quelle — einmal für die Bühne, einmal für die Datei. Was man
sieht, kommt heraus.

Felder in geschweiften Klammern: `{seite}` `{seiten}` `{datum}` `{zeit}`
`{datei}` `{titel}`. Die Zählung ist nicht die Seitenzahl: „ab Seite 3,
Zählung beginnt bei 1" heißt, dass die dritte Seite die Eins trägt und die
Gesamtzahl entsprechend kleiner ist — Deckblatt und Inhaltsverzeichnis zählen
in Gremienunterlagen nicht mit.

### Felderkennung — ein flaches Formular ausfüllbar machen

`Werkzeuge → Formularfelder erkennen`. Gesucht wird **im gerenderten Bild**,
nicht in den Zeichenbefehlen. Das ist eine Entscheidung mit Folgen: die
Zeichenbefehle sauber auszulesen hieße, den halben PDF-Zeichner nachzubauen,
und ein Scan hat überhaupt keine. So findet dieselbe Suche die Felder im
gesetzten wie im eingescannten Formular.

| Was | Woran erkannt | Wird zu |
|---|---|---|
| Ausfülllinie | waagerechter Strich ab 60 pt, darüber Platz | Textfeld darüber |
| Kästchen | Quadrat von 6 bis 22 pt | Ankreuzfeld |
| Rahmen | Rechteck ab 60 × 12 pt | Textfeld, über 45 pt Höhe mehrzeilig |
| „Unterschrift:" | Beschriftung an einer langen Linie | Unterschriftsfeld |

**Eine Fläche, in der schon etwas steht, gilt nicht als Feld.** Diese eine
Regel trennt Formular von Layout — ohne sie schlägt die Erkennung auf jeder
Tabelle hunderte Felder vor. Der Prüflauf hält das fest: das Testformular
enthält eine gefüllte Tabelle, und keine ihrer Zellen darf vorgeschlagen
werden.

Angelegt wird nichts von selbst. Die Erkennung schlägt vor, der Mensch
bestätigt, und jeder Fund nennt den Grund, aus dem er gefunden wurde.

### Absätze — Bearbeiten, das umbricht

Ein PDF kennt keine Absätze, nur Buchstabengruppen an Punkten. `absaetze.js`
rechnet sie zusammen: Läufe auf einer Grundlinie werden zur Zeile, Zeilen mit
gleichem Schriftgrad, gleichem linken Rand und gleichmäßigem Abstand werden
zum Absatz. Decken sich links und rechts, ist es Blocksatz.

Damit fasst ein Klick mit **Text bearbeiten (B)** den ganzen Absatz, und der
neue Text wird umgebrochen statt an derselben Stelle abgeschnitten. Vor dem
Übernehmen steht da, wieviele Zeilen es werden — ein Absatz, der wächst,
überdeckt, was darunter steht.

Mit **Alt beim Klicken** bleibt es beim einzelnen Textlauf. Für eine Zahl in
einer Tabellenzelle ist der Absatz zu viel.

Was dabei verloren geht, und zwar mit Absicht: fette und kursive Stellen
innerhalb des Absatzes. Beim Umbrechen verschieben sie sich, und geraten ist
...
Adresszeile, Symbol im Dock oder Startmenü, und es läuft ohne Verbindung.
...
Wer das Studio selbst betreiben will, kopiert den Ordner und stellt einen
...
**Anmeldung und Offline.** Erreicht das Studio die Auskunft nicht, entscheidet
