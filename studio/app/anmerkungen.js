...
export function zeichneAnmerkungen(ebene, eintrag, sicht) {
  ...
  for (const a of zustand.anmerkungen.filter((a) => a.seiteId === eintrag.id)) {
    ...
    if (a.art === 'ersatz') {
      ...
      /* Ein Absatz (mit `zeilenhoehe`) bricht auch auf dem Bildschirm um —
         sonst zeigt die Bühne eine Zeile und die Datei drei. Ein einzelner
         Lauf bleibt einzeilig, wie er es in der Ausgabe auch tut. */
      const alsAbsatz = !!a.zeilenhoehe;
      const knoten = el('div', {
        ...
        stil: {
          ...
          minWidth: `${breite}px`,
          ...(alsAbsatz
            ? { width: `${breite}px`, minHeight: `${hoehe}px`, lineHeight: `${a.zeilenhoehe * sicht.scale}px`, whiteSpace: 'pre-wrap' }
            : { height: `${hoehe}px`, lineHeight: `${hoehe}px`, whiteSpace: 'pre' }),
          background: a.grundfarbe || '#fff', color: a.schriftfarbe || '#111',
          fontSize: `${a.groesse * sicht.scale}px`,
          textAlign: a.ausrichtung === 'mitte' ? 'center' : a.ausrichtung === 'rechts' ? 'right' : 'left',
...
export function starteWerkzeuge(spur, zuPdfPunkt) {
  ...
  /* Text bearbeiten: Klick auf ein Stück der Textebene. Was dabei
     herauskommt, ist die Beschreibung des einzelnen Laufs — welchen Absatz er
     bildet, rechnet der Dialog aus, weil er dafür die Seite lesen muss und
     das nicht in einen Klickhandler gehört. Mit gedrückter Alt-Taste bleibt
     es beim einzelnen Lauf: für eine Zahl in einer Tabelle ist der ganze
     Absatz zu viel. */
  spur.addEventListener('click', (ereignis) => {
    ...
    melde('anmerkung:textErsetzen', {
      ...beschreibeTextstueck(stueck, blattKnoten),
      nurDieserLauf: ereignis.altKey,
    });
  });
