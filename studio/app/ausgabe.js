  ...
  for (const eintrag of folge) {
    ...
    for (const a of anmerkungen.filter((x) => x.seiteId === eintrag.id)) {
      ...
      if (a.art === 'hervor') {
        ...
        /* Überdecken, dann neu setzen. Ein Absatz bekommt dabei seinen
           Umbruch neu gerechnet — das ist der Unterschied zum Ersetzen eines
           einzelnen Laufs, bei dem `zeilenhoehe` fehlt und genau eine Zeile
           herauskommt. */
        ...
        const zeilenhoehe = a.zeilenhoehe || groesse * 1.2;
        const zeilen = a.zeilenhoehe
          ? umbreche(a.text, schrift, groesse, a.b)
          : [nurWinAnsi(a.text)];
        /* Wird der Text länger, wächst der Kasten nach unten. Der Dialog hat
           vorher gesagt, wieviele Zeilen es werden — hier wird es getan. */
        const hoehe = Math.max(a.h, (zeilen.length - 1) * zeilenhoehe + groesse * 1.1);
        seite.drawRectangle({
          x: a.x + vx - rand, y: a.y + vy + a.h - hoehe - rand * 0.5,
          width: a.b + rand * 2, height: hoehe + rand,
          color: rgb(grund.r, grund.g, grund.b),
        });
        try {
          /* Die erste Grundlinie liegt dort, wo sie vorher lag: der Absatz
             soll oben stehen bleiben und nach unten wachsen, nicht wandern. */
          let y = a.y + vy + a.h - groesse * 0.84;
          for (const zeile of zeilen) {
            const text = nurWinAnsi(zeile);
            const breite = schrift.widthOfTextAtSize(text, groesse);
            const x = a.ausrichtung === 'mitte' ? a.x + vx + (a.b - breite) / 2
              : a.ausrichtung === 'rechts' ? a.x + vx + a.b - breite
                : a.x + vx;
            seite.drawText(text, {
              x, y, size: groesse, font: schrift,
              color: rgb(schriftfarbe.r, schriftfarbe.g, schriftfarbe.b),
            });
            y -= zeilenhoehe;
          }
...
/* ---------- Aufdruck: Wasserzeichen, Kopf, Fuß, Seitenzahlen --------------- */

/* Gerechnet wird in `aufdruck.js`, hier wird nur gezeichnet. Dieselben
   Befehle, die `ansicht.js` auf den Bildschirm malt, landen so in der Datei —
   ein Wasserzeichen, das man sieht, aber nicht bekommt, wäre schlimmer als
   keins. */
async function maleAufdruck(ziel, folge, versatzKarte) {
  const { befehleFuerSeite } = await import('./aufdruck.js');
  if (!zustand.aufdruck) return;
  const { rgb, StandardFonts, degrees } = pdflib;
  const schrift = await ziel.embedFont(StandardFonts.Helvetica);
  const fett = await ziel.embedFont(StandardFonts.HelveticaBold);

  for (const [i, eintrag] of folge.entries()) {
    const ort = versatzKarte.get(eintrag.id);
    if (!ort) continue;
    const { seite, versatz } = ort;
    const { width, height } = seite.getSize();
    const befehle = befehleFuerSeite({
      nummer: i + 1, seitenzahl: folge.length, breitePt: width, hoehePt: height,
    });

    for (const b of befehle) {
      const stift = b.art === 'schraeg' ? fett : schrift;
      const breite = stift.widthOfTextAtSize(b.text, b.groesse);
      /* Waagerecht ausrichten. Beim schrägen Wasserzeichen dreht pdf-lib um
         den Anfangspunkt, also muss die halbe Breite in der gedrehten
         Richtung abgezogen werden — sonst hängt es rechts unten am Rand. */
      let x = b.x, y = b.y;
      if (b.art === 'schraeg') {
        const bogen = (b.winkel * Math.PI) / 180;
        x -= (breite / 2) * Math.cos(bogen);
        y -= (breite / 2) * Math.sin(bogen);
        y -= (b.groesse * 0.35) * Math.cos(bogen);
      } else if (b.ausrichtung === 'mitte') x -= breite / 2;
      else if (b.ausrichtung === 'rechts') x -= breite;

      const f = farbeZuAnteilen(b.farbe);
      seite.drawText(b.text, {
        x: x - versatz.x,
        y: y - versatz.y,
        size: b.groesse,
        font: stift,
        ...
        opacity: b.deckung,
        rotate: b.winkel ? degrees(b.winkel) : undefined,
      });
    }
  }
}
