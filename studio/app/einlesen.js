/* Einlesen — aus Word, Excel, Text, Markdown und CSV wird ein PDF.
   ...
   **Was dabei nicht ankommt, und warum:** Word beschreibt ein Dokument mit
   Vorlagen, Nummerierungen, Rahmen, Feldern und Zeichnungen. Das Studio
...
export async function ausDatei(datei, optionen = {}) {
  ...
  if (!bloecke.length) throw new Error(`${name} enthält keinen Text, den das Studio setzen könnte.`);
...
export async function liesBloecke(bytes, name) {
  ...
  throw new Error(`${name}: dieses Format kann das Studio nicht einlesen.`);
}
