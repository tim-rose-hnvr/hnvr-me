...
const GEBAUT = resolve(HIER, '..', '..', 'portal', 'public', 'studio');
...
console.log('\n== Anmeldung ==');
{
  ...
  const anwendung = await hole('/studio/index.html');
  pruefe(/name="studio-anmeldung"/.test(anwendung.koerper.toString('utf8')),
...
console.log('\n== Installierbar und offline-fest ==');
{
  ...
  const manifest = await hole('/studio/manifest.json');
  let gelesen = null;
  ...
  /* Ein Dienst, der über sein Verzeichnis hinaus greifen dürfte, wäre ein
     Fehler — der Umfang ergibt sich aus dem Ort, und der ist /studio/. */
  pruefe(/scope: '\.\/'/.test(dienst.koerper.toString('utf8'))
    || /register\('dienst\.js'/.test((await hole('/studio/app/installieren.js')).koerper.toString('utf8')),
...
await Promise.all(Array.from({ length: reihen }, async () => {
  while (stapel.length) {
    ...
    const weg = `/studio/${relative(GEBAUT, pfad).split('\\').join('/')}`;
