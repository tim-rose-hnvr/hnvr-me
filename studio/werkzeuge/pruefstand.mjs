/* Prüfstand — der Aufbau, den alle Prüfungen teilen.
   ...
   Ein Server auf das Studio, ein Browser darauf, eine Seite darin, und
...
export async function starte() {
  ...
  const HIER = await mkdtemp(join(tmpdir(), 'studio-vollpruefung-'));
    ...
    try {
      ...
      try {
        ...
        const lage = await seite.evaluate(() => ({
          ...
          werkzeug: window.studio.zustand.werkzeug,
          seite: window.studio.zustand.aktuelleSeite,
          zoom: String(window.studio.zustand.zoom),
          anmerkungen: window.studio.zustand.anmerkungen.map((a) => a.art),
        }));
  ...
  const ablage = await mkdtemp(join(tmpdir(), 'studio-knoepfe-'));
  ...
  await ladeBeispiel();
  const befehle = await seite.evaluate(() => window.studio.befehle.map((b) => ({ id: b.id, name: b.name, gruppe: b.gruppe, kuerzel: b.kuerzel })));
