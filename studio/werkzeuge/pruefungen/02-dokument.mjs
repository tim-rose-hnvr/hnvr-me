...
export default async function ({ pruefe, melde, seite, blatt, ladungVon, dialogSchliessen, HIER, WURZEL }) {
  ...
  await pruefe('Alle Seiten wählen / keine', async () => {
    ...
    await seite.evaluate(() => window.studio.fuehreAus('seiten:alleWaehlen'));
    await seite.waitForTimeout(300);
    const alle = await seite.evaluate(() => window.studio.zustand.gewaehlteSeiten.size);
    await seite.evaluate(async () => {
      const { melde } = await import('./app/kern.js');
      window.studio.zustand.gewaehlteSeiten.clear();
      melde('auswahl:geaendert');
    });
    await seite.waitForTimeout(300);
    const keine = await seite.evaluate(() => window.studio.zustand.gewaehlteSeiten.size);
  ...
  await pruefe('Seiten verdoppeln', async () => {
    await seite.evaluate(() => {
      ...
      window.studio.fuehreAus('seiten:verdoppeln');
    });
    await seite.waitForTimeout(900);
    const n = await seite.evaluate(() => window.studio.zustand.folge.length);
    if (n !== 6) throw new Error(String(n));
    await seite.evaluate(() => window.studio.fuehreAus('rueckgaengig'));
    await seite.waitForTimeout(800);
    return 'zurückgenommen auf ' + await seite.evaluate(() => window.studio.zustand.folge.length);
  });
  await pruefe('Miniaturen umsortieren (Ziehen)', async () => {
    ...
    await seite.evaluate(() => window.studio.fuehreAus('rueckgaengig'));
    await seite.waitForTimeout(900);
    const zurueck = await seite.evaluate(() => window.studio.zustand.folge.map((e) => e.id));
  ...
  await pruefe('Gewählte Seiten als neue Datei', async () => {
    ...
    const pfad = await ladungVon(() => seite.evaluate(() => window.studio.fuehreAus('seiten:ausgeben')));
  ...
  await pruefe('Treffer anklicken springt zur Stelle', async () => {
    ...
    const seitenNr = await seite.evaluate(() => window.studio.zustand.aktuelleSeite);
  ...
  await pruefe('Ankreuzfeld und Auswahlliste', async () => {
    ...
    await seite.waitForTimeout(400);
    const werte = await seite.evaluate(() => Object.fromEntries(window.studio.zustand.formularwerte));
  ...
  await pruefe('Formularwerte landen in der Ausgabe', async () => {
    const pfad = await ladungVon(() => seite.evaluate(() => window.studio.fuehreAus('sichern:einbrennen')));
  ...
  await pruefe('Text ausgeben (.txt)', async () => {
    const pfad = await ladungVon(() => seite.evaluate(() => window.studio.fuehreAus('text:ausgeben')));
  ...
  await pruefe('Seite als PNG', async () => {
    const pfad = await ladungVon(() => seite.evaluate(() => window.studio.fuehreAus('bild:ausgeben')));
  ...
  await pruefe('Teilen in Einzeldateien', async () => {
    ...
    await seite.evaluate(() => window.studio.fuehreAus('teilen'));
  ...
  await pruefe('Reparieren', async () => {
    const pfad = await ladungVon(() => seite.evaluate(() => window.studio.fuehreAus('reparieren')));
  ...
  await pruefe('Linearisieren', async () => {
    const pfad = await ladungVon(() => seite.evaluate(() => window.studio.fuehreAus('linearisieren')));
  ...
  await pruefe('Schutz und Rechte anzeigen', async () => {
    await seite.evaluate(() => window.studio.fuehreAus('schutz:zeigen'));
  ...
  await pruefe('Verkleinern (Dialog)', async () => {
    await seite.evaluate(() => window.studio.fuehreAus('verkleinern'));
  ...
  await pruefe('Befehlspalette führt Befehl aus', async () => {
    ...
    await seite.waitForTimeout(900);
    const zoom = await seite.evaluate(() => window.studio.zustand.zoom);
  ...
  await pruefe('Muster-Fundstellen', async () => {
    await seite.evaluate(() => window.studio.fuehreAus('muster:zeigen'));
    await seite.waitForSelector('.dialog');
  ...
  await pruefe('Seiten ohne Text wählen', async () => {
    await seite.evaluate(() => window.studio.fuehreAus('leere:waehlen'));
    await seite.waitForTimeout(600);
    const n = await seite.evaluate(() => window.studio.zustand.gewaehlteSeiten.size);
    await seite.evaluate(() => { window.studio.zustand.gewaehlteSeiten.clear(); });
  ...
  await pruefe('Unterschrift tippen', async () => {
    await seite.evaluate(() => window.studio.fuehreAus('unterschrift:anlegen'));
    ...
    const b = await blatt();
    await seite.evaluate(() => window.studio.fuehreAus('werkzeug:unterschrift'));
    ...
    await seite.waitForTimeout(500);
    const u = await seite.evaluate(() => window.studio.zustand.anmerkungen.find((a) => a.art === 'unterschrift'));
  ...
  await pruefe('Unterschrift aus Bilddatei', async () => {
    ...
    await seite.mouse.up();
    await seite.waitForTimeout(500);
    const anzahl = await seite.evaluate(() => window.studio.zustand.anmerkungen.filter((a) => a.art === 'unterschrift').length);
  ...
  await pruefe('Drucken reicht die erzeugte Datei an den Betrachter', async () => {
    // Geprüft wird die Zusage des Studios — eine fertige PDF-Datei zu öffnen.
    ...
    const adresse = await seite.evaluate(async () => {
      ...
      try {
        await window.studio.fuehreAus('drucken');
