/* Installieren — aus der Seite eine Anwendung machen.
   ...
   1. **Den Dienst anmelden.** Danach läuft die Werkbank ohne Netz. Wird eine
      neue Fassung veröffentlicht, meldet sich der Dienst und die Werkbank
      bietet an, neu zu starten — sie tut es nicht von selbst, weil dabei ein
   ...
   4. **Dateien vom Betriebssystem annehmen.** Eine installierte Werkbank darf
   ...
   `launchQueue` läuft die Werkbank unverändert. Das ist Absicht — sie darf von
...
function hoerAufsAngebot() {
  ...
  addEventListener('appinstalled', () => {
    ...
    sage('Die Werkbank ist installiert — sie steht jetzt im Startmenü.', { dauer: 6000 });
  });
}
...
async function meldeDienstAn() {
  ...
  anmeldung.addEventListener('updatefound', () => {
    ...
    neuer.addEventListener('statechange', () => {
      ...
      sage('Eine neue Fassung der Werkbank liegt bereit.', {
        dauer: 0,
...
/* Dateien, die das Betriebssystem hereinreicht: „Öffnen mit → Werkbank".
...
/* Ein Wort in der Fußzeile: läuft die Werkbank offline-fest oder nicht. Es
...
export function zeichneStand(behaelter) {
  ...
  behaelter.append(el('span', {
    text: laeuft ? (istInstalliert() ? 'installiert, läuft offline' : 'läuft offline')
      : 'nur mit Netz',
    title: laeuft
      ? 'Die Werkbank ist auf diesem Gerät hinterlegt und startet ohne Verbindung.'
      : 'Es ist kein Dienst angemeldet — ohne Verbindung startet die Werkbank nicht.',
  }));
}
