...
export default async function ({ pruefe, seite, ladeBeispiel, WURZEL, BASIS }) {
  ...
  await pruefe('Der Dienst meldet sich an und trägt die Werkbank offline', async () => {
    await ladeBeispiel();
  ...
  await pruefe('Der Einrichten-Dialog sagt die Wahrheit über diesen Browser', async () => {
    await seite.evaluate(() => window.werkbank.fuehreAus('installieren'));
  ...
  await pruefe('Startbefehle aus der Adresse greifen', async () => {
    ...
    const name = await seite.evaluate(() => window.werkbank.zustand.name);
